...
public class SharedStreamBenchmark {

  private static final Logger LOGGER = LoggerFactory.getLogger(SharedStreamBenchmark.class);

  private static final String QUALIFIER = "/benchmarks/sharedStream";
              ...
              return service ->
                  (executionTick, task) -> {
                    Flux<ServiceMessage> requestStream =
                        service
                            .sharedStream(message) //
                            .map(SharedStreamBenchmark::enichResponse);
                    if (rateLimit != null) {
