...
public class ClusterBootstrapExamples {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    ClusterConfiguration config5 =
        ClusterConfiguration.newInstance().port(4004).seedMembers(seedMembers).memberId(
            "Another cool member").metadata(metadata);
    ICluster cluster5 = Cluster.newInstance(config5).join();

    // If you don't want to call join
    ClusterConfiguration config6 = ClusterConfiguration.newInstance().port(4005).seedMembers(
        seedMembers).autoJoin(true);
    ICluster cluster6 = Cluster.newInstance(config6);

    // Alone cluster member - trying to join, but always ignored :(
    ClusterConfiguration.ClusterMembershipSettings membershipSettings7 = new ClusterConfiguration.ClusterMembershipSettings();
    membershipSettings7.setSyncGroup("forever alone");
    ClusterConfiguration config7 =
        ClusterConfiguration.newInstance().port(4006).seedMembers(
            seedMembers).clusterMembershipSettings(membershipSettings7);
