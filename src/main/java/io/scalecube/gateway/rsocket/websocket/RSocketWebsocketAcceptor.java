  ...
  public Mono<RSocket> accept(ConnectionSetupPayload setup, RSocket rSocket) {
    ...
    return Mono.just(new AbstractRSocket() {
      ...
      public Mono<Void> fireAndForget(Payload payload) {
        ...
        Timer.Context roundtripTime = metrics.getTimer(METRICS_PREFIX, METRIC_STREAM_DURATION).time();
        return serviceCall
            .oneWay(toServiceMessage(payload))
            .doOnTerminate(roundtripTime::stop);
      }

      @Override
      public Mono<Payload> requestResponse(Payload payload) {
        metrics.getMeter(METRICS_PREFIX, METRIC_CLIENT, METRIC_REQUESTS).mark();
        Timer.Context streamDuration = RSocketWebsocketAcceptor.this.metrics.getTimer(METRICS_PREFIX, METRIC_STREAM_DURATION).time();
        return serviceCall
            .requestOne(toServiceMessage(payload))
            .map(this::toPayload)
            .doOnNext(n -> metrics.getMeter(METRICS_PREFIX, METRIC_CLIENT, METRIC_RESPONSES).mark())
      ...
      public Flux<Payload> requestChannel(Publisher<Payload> payloads) {
        Timer.Context streamDuration = RSocketWebsocketAcceptor.this.metrics.getTimer(METRICS_PREFIX, METRIC_STREAM_DURATION).time();
        final Publisher<ServiceMessage> publisher = Flux
            .from(payloads)
            .doOnNext(n -> metrics.getMeter(METRICS_PREFIX, METRIC_CLIENT, METRIC_REQUESTS).mark())
