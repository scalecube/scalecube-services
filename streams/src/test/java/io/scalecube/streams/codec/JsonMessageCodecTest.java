...
public class JsonMessageCodecTest {
  ...
  public void testEncodeObj() throws Exception {
    ...
    // When:
    StreamMessage encoded = codec.encode(msg);
    // Then:
  ...
  public void testDecodeObj() throws Exception {
    // Given
    String payload = "Test";
    StreamMessage encoded = codec.encode(StreamMessage.builder().qualifier(TEST).data(payload).build());
    // When:
    StreamMessage decoded = codec.decode(encoded, String.class);
    // Then:
  ...
  public void testComplexObjectSerDeser() throws IOException {
    ...
    // When
    StreamMessage encoded = codec.encode(StreamMessage.builder().qualifier(TEST).data(payload).build());
    StreamMessage decoded = codec.decode(encoded, TestPayload.class);
