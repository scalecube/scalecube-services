...
public interface ITransport {
	...
	/**
	 * Starts transport on the given endpoint so it is started to accept connection
	 * and can connect to other endpoint.
	 */
	void start();
	...
	/**
	 * Returns stream of messages received from any remote endpoint regardless direction of connection.
	 * For each observers subscribed to the returned observable:
	 * <ul>
	 * <li>{@link rx.Observer#onNext(Object)} will be invoked when some message arrived to current endpoint</li>
	 * <li>{@link rx.Observer#onCompleted()} will be invoked when there is no possibility that server will receive new message
	 * observable for already closed transport</li>
	...
	/**
	 * Stop transport, disconnect all available connections which belong to this transport.
	 * <br/>
	 ...
	 * immediately emit onComplete event for all subscribers.
	 * <br/>
	 * Stop is async operation, if result of operation is not needed leave second parameter null,
