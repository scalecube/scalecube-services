...
/**
 * Represent abstraction over p2p duplex connection between two transport endpoints. Allows sending messages and listen incoming messages.
 */
public interface ITransportChannel {
  ...
  /**
   * Sends message to remote endpoint. Send is async operation, if result of operation is not needed leave second parameter null, otherwise
   * pass {@link SettableFuture}. If transport channel is already closed - {@code promise} will be failed with
   * {@link TransportClosedException}.
   * 
   * @param message message to send
   ...
   * Close is async operation, if result of operation is not needed leave second parameter null, otherwise pass {@link SettableFuture}.
   * 
