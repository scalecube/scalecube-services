...
public class LocalChannelTransportFactoryTest {
	static final Logger LOGGER = LoggerFactory.getLogger(LocalChannelTransportFactoryTest.class);
	...
	public void testInteractWithNoConnection() throws Exception {
		...
		try {
			...
			ChannelException cause = (ChannelException) e.getCause();
		}
	...
	public void testPingPongOnSingleChannel() throws Exception {
		TransportEndpoint serverEndpoint = from("local://server");
		TransportEndpoint clientEndpoint = from("local://client");
	...
	public void testNetworkSettings() throws InterruptedException {
		...
		server = TF(serverEndpoint);
		client = TF(from("local://client"));

		int lostPercent = 50;
		int mean = 0;
		client.<LocalChannelPipelineFactory> getPipelineFactory().setNetworkSettings(serverEndpoint, lostPercent, mean);
	...
	private TransportEndpoint serverEndpoint() {
		return from("local://server");
	}

	private TransportEndpoint clientEndpoint() {
		return from("local://client");
	}
			...
			public void run() {
				if (f.isDone()) {
					try {
						...
						LOGGER.warn("Failed to send {} to {}, transport: {}, cause: {}", msg, to, transport, e.getCause());
					}
				}
			}
		}, MoreExecutors.directExecutor());
		transport.send(msg, f);
	}
	...
	private Transport TF(TransportEndpoint endpoint, Map localMetadata, int sendHwm) {
		...
		tf.setPipelineFactory(new LocalChannelPipelineFactory());
		tf.start();
		return tf;
	}
